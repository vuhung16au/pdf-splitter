...
# Remove references to network throttling/mobile tests if any, as the test file is being removed/emptied
...
if [[ "$TEST_TYPE" == "all" || "$TEST_TYPE" == "e2e" ]]; then
  ...
  if [[ "$RUN_BROWSER_COMPAT" == "true" ]]; then
    ...
    for browser in "${browsers[@]}"; do
      ...
      if [[ "$TARGET" == "production" ]]; then
        ...
        # Run against localhost (do NOT start dev server)
        npx cypress run --config baseUrl=http://localhost:3000/ --browser $browser --spec cypress/e2e/browser-compatibility.cy.ts
      fi
  ...
  if [[ "$TARGET" == "production" ]]; then
    ...
    # Run against localhost (do NOT start dev server)
    npx cypress run --config baseUrl=http://localhost:3000/ --browser $BROWSER
  fi
  ...
  #       if $is_ci; then
  #         npx start-server-and-test dev http://localhost:3001 "cypress run --browser $BROWSER --spec $test_file"
  #       else
  #         npx start-server-and-test dev http://localhost:3001 "cypress run --browser $BROWSER --spec $test_file"
  #       fi
  #     fi
  ...
  #     # Run against localhost
  #     if $is_ci; then
  #       npx start-server-and-test dev http://localhost:3001 "cypress run --browser $BROWSER --spec cypress/e2e/performance.cy.ts"
  #     else
  #       npx start-server-and-test dev http://localhost:3001 "cypress run --browser $BROWSER --spec cypress/e2e/performance.cy.ts"
  #     fi
  #   fi
...
#       # Run against localhost
#       if $is_ci; then
#         npx start-server-and-test dev http://localhost:3001 "cypress run --browser $browser --spec cypress/e2e/accessibility.cy.ts"
#       else
#         npx start-server-and-test dev http://localhost:3001 "cypress run --browser $browser --spec cypress/e2e/accessibility.cy.ts"
#       fi
#     fi
...
#     # Run against localhost
#     if $is_ci; then
#       # npx start-server-and-test dev http://localhost:3001 "cypress run --browser $BROWSER --spec cypress/e2e/security/csp.cy.ts"
#     else
#       # npx start-server-and-test dev http://localhost:3001 "cypress run --browser $BROWSER --spec cypress/e2e/security/csp.cy.ts"
#     fi
#   fi
...
#     # Run against localhost
#     if $is_ci; then
#       # npx start-server-and-test dev http://localhost:3001 "cypress run --browser $BROWSER --spec cypress/e2e/security/xss.cy.ts"
#     else
#       # npx start-server-and-test dev http://localhost:3001 "cypress run --browser $BROWSER --spec cypress/e2e/security/xss.cy.ts"
#     fi
#   fi
...
#     # Run against localhost
#     if $is_ci; then
#       npx start-server-and-test dev http://localhost:3001 "cypress run --browser $BROWSER --spec cypress/e2e/security/file-upload.cy.ts"
#     else
#       npx start-server-and-test dev http://localhost:3001 "cypress run --browser $BROWSER --spec cypress/e2e/security/file-upload.cy.ts"
#     fi
#   fi
...
if [[ "$TEST_TYPE" == "all" ]]; then
  ...
  if [ ${component_status:-0} -eq 0 ] && [ ${e2e_status:-0} -eq 0 ] && [ ${a11y_status:-0} -eq 0 ] && [ ${mobile_status:-0} -eq 0 ]; then
    exit 0
  else
    exit 1
  fi
...
# Ensure all open conditionals are closed
# Add a final 'fi' to close any unclosed block
fi
