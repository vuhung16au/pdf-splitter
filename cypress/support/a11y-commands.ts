...
Cypress.Commands.add('tabUntil', { prevSubject: 'optional' }, (subject, selector, options = {}) => {
  const {
    maxNumberOfTabs = 10, // Default limit of 10 tabs
    timeout = 4000, // Default timeout of 4s
  } = options;

  function tabAndCheck(iteration = 0) {
...
Cypress.Commands.add('hasAccessibleName', { prevSubject: true }, (subject) => {
...
Cypress.Commands.add('isKeyboardAccessible', { prevSubject: true }, (subject) => {
  return cy.wrap(subject).then($el => {
